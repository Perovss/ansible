...
function register {
    ...
    curl -k --location --silent --show-error --max-time "${CURL_MAX_TIME:=5}" --header "$CURL_TOKEN_HEADER" --request PUT --data "$JSON" "$CONSUL_URL/v1/agent/service/register"
}
...
function deregister {
    ...
    ID="${ENV}_${HOSTNAME}_${NAME}"
    curl -k --location --silent --show-error --max-time "${CURL_MAX_TIME:=5}" --header "$CURL_TOKEN_HEADER" --request PUT "$CONSUL_URL/v1/agent/service/deregister/$ID"
}
